...
from p3_generatePathPoints import remove_dir,check_dir,generate_osm_routes_main
...
def create_geometry_from_latlon(table_name):
	conn= connect()
	cur = conn.cursor()
	update_sql = "UPDATE "+ table_name +" SET geom = ST_SetSRID(ST_MakePoint(lon, lat), 4326);"
...
l5=Label(win,text="Retain Samples (%)")
